...
class CountDown:
    def __init__(self, root):
        ...
        self.window.configure(bg='#14d9d9')
        ...
        self.button_frame = Frame(self.window, bg="#14d9d9", width=240, height=40)
        self.button_frame.place(x=230, y=150)

        # This frame is used to show the countdown time label
        self.time_frame = Frame(self.window, bg="#14d9d9", width=480, height=120).place(x=0, y=210)

        # Tkinter Labels
        time_label = Label(self.window, text="Set Time",
                           font=("times new roman", 22, "bold"), bg='#14d9d9', fg='blue')
        time_label.place(x=180, y=30)

        hour_label = Label(self.window, text="Hour",
                           font=("times new roman", 18), bg='#14d9d9', fg='black')
    ...
    # When the set button is clicked, this function gets called

    def Get_Time(self):
        self.time_display = Label(self.time_frame,
                                  font=('Helvetica', 20, "bold"),
                                  bg='#14d9d9', fg='black')
        self.time_display.place(x=130, y=225)

        try:
            # Total amount of time in seconds
            hour = (int(self.hour_combobox.get()) * 3600)
            minute = (int(self.minute_combobox.get()) * 60)
            second = (int(self.second_combobox.get()))
            self.time_left = hour + minute + second
    ...
    def start_time(self):
        ...
        while self.time_left > 0:
            ...
            # When the time is over, a piece of music will be played  in the background
